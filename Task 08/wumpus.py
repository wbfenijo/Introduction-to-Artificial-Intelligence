...
def choose_action_automatic(world, percept):
    # Automatic agent. Should be able to pass the
    # User inputs: "[move|shoot|climb], direction". E.g "move up".
    print('You are at position {} and you percept: {}'.format(world.hero, (percept if percept else '{}')))
    kb_add_step(world, percept)
    #print_safe_moves(world)
    ...
    if 'stench' in percept and not world.killed_wumpus:
        ...
        for direction, dr, dc in moves:
            nr, nc = r + dr, c + dc
            if 0 <= nr < len(world.mapa) and 0 <= nc < len(world.mapa[0]):
                if world.mapa[nr][nc]['type'] != 'wall':
                    candidates.append((direction, nr, nc))

        remaining = []
        for direction, nr, nc in candidates:
            if not kb.ask(NOT('wumpus_at', nr, nc)):
                remaining.append((direction, nr, nc))
...
def kb_initialize(knowledge_base, world):
    ...
    for r in range(len(world.mapa)):
        for c in range(len(world.mapa[0])):
            ...
            knowledge_base.tell(Implication([NOT('wumpus_at', r, c),
                                             NOT('pit_at', r, c)], L('safe', r, c)))
            for dr, dc in [(-1, 0), (0, 1), (1, 0), (0, -1)]:
                # There`s no Wumpus if there`s no stench besides
                knowledge_base.tell(Implication([NOT('stench', r+dr, c+dc)],
                                                NOT('wumpus_at', r, c)))
                # There`s no pit if there`s no breeze besides
                knowledge_base.tell(Implication([NOT('breeze', r+dr, c+dc)],
                                                NOT('pit_at', r, c)))
